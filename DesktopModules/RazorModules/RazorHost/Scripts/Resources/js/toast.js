...
(function (global) {
    ...
    function show(message, ok) {
        ...
        toast.className = `fixed bottom-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium transition-opacity duration-300 max-w-[350px] ${ok ? 'bg-success-50 border border-success-200 text-success-800' : 'bg-danger-50 border border-danger-200 text-danger-800'}`;
        toast.innerHTML = `<i data-lucide="${ok ? 'check-circle-2' : 'alert-circle'}" class="size-4 shrink-0"></i><span>${escHtml(message)}</span>`;
